...
class Auth:
    ...
    CLIENT_ID = "00000000402B5328"#警告：正版登录时不要使用这个 Client ID，这个是仅限测试用的，如果你将其用作游戏，将会违反微软的政策。∵虽然它免费，但是还是需要绑银行卡，而我的银行卡不被微软接受，∴我无法申请 Client ID，请你们自行申请。必须将其换成你自己的 Client ID。在 https://portal.azure.com 上可以免费申请 Client ID，不会可以问AI。CML、CML 的作者对用户因使用测试 Client ID 进行游戏而导致的账号封禁或其他任何后果不承担任何责任。
    TENANT = "consumers"
...
class CLI:
    ...
    def _save_auth(self):
        ...
        with open(self.state.auth_file, 'w') as f:
            json.dump(data, f, indent=2)
        os.chmod(self.state.auth_file, 0o600)  # 仅所有者可读写
...
def print_banner(state):
    ...
    if state.username and AccountStore.account_exists(state.username, state.accounts_dir):
        ...
        try:
            ...
            pass
    print(f"""{C.BRIGHT_BLUE}{C.BOLD}Chen Minecraft Launcher 8 (CML8) ©2026 童顺
...
if __name__ == "__main__":
    print("警告：代码第 161 行有重要信息，使用本工具即表示你已阅读并理解此警告。将其阅读并理解后可将代码第4603行注释或删除，以消除此警告")
    main()